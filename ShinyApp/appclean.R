...
#------------------------------------------------------------------------------#
  ...
  allchemicals<-eventReactive(input$run_button, {for (x in 1:nrow(iondat$dat)){
    ...
    #allconcdf<-cbind(timeframe(), allconcdf)
    allconcdf
  })
...
# nglframe<-reactive({data.frame(matrix(1000, nrow=nrow(allchemicals()), ncol=ncol(allchemicals())))})
# uglframe<-reactive({data.frame(matrix(1000000, nrow=nrow(allchemicals()), ncol=ncol(allchemicals())))})

#concconversions<-reactive({reactiveValues(cc0=0, mgl=0, ngl=0, ugl=0)
#concconversions$mgl<-reactive({alldata()})
#concconversions$ngl<-reactive({alldata()*1000})
#output$sum3<-renderTable(concconversions()$mgl)
# 
# dayframe<-reactive({data.frame(matrix(24, nrow=nrow(allchemicals()), ncol=ncol(allchemicals())))})
# monthframe<-reactive({data.frame(matrix(24, nrow=nrow(allchemicals()), ncol=ncol(allchemicals())))})
# yearframe<-reactive({data.frame(matrix(24, nrow=nrow(allchemicals()), ncol=ncol(allchemicals())))})
# bedframe<-reactive({data.frame(matrix(bedvolume(), nrow=nrow(allchemicals()), ncol=ncol(allchemicals())))})
...
#observe({outputallconcs(allchemicals())})
#outallconcsholder<-reactiveValues(chemicals=0)
...
#output$sum<-renderTable(outputallconcs$chemicals)
output$sum<-renderTable(convertedframes$mgl)
output$sum2<-renderTable(convertedframes$ngl)
output$sum3<-renderTable(convertedframes$ugl)
output$sum4<-renderTable(convertedframes$cc0)
